...
const buf = fs.readFileSync("./__tests__/data/air.hdr.gz");
let data = utilities_1.Utils.toArrayBuffer(buf);
let nifti1;
const ibuf = fs.readFileSync('./__tests__/data/air.img.gz');
...
describe('NIFTI-Reader-JS', function () {
    describe('uncompressed nifti-1 hdr/img pair test', function () {
        ...
        it('isNIFTI1() should return true', function () {
            chai_1.assert.equal(true, (0, nifti_1.isNIFTI1)(data, true));
        });
        ...
        it('should not throw error when reading header', function (done) {
            chai_1.assert.doesNotThrow(function () {
                nifti1 = (0, nifti_1.readHeader)(data, true);
                done();
            });
        });
        it('dims[1] should be 79', function () {
            chai_1.assert.equal(79, nifti1.dims[1]);
        });
        it('dims[2] should be 67', function () {
            chai_1.assert.equal(67, nifti1.dims[2]);
        });
        it('dims[3] should be 64', function () {
            chai_1.assert.equal(64, nifti1.dims[3]);
        });
        it('image data checksum should equal 692149477', function () {
            var imageData = (0, nifti_1.readImage)(nifti1, idata);
...
//# sourceMappingURL=nifti1-pair.spec.js.map